...
import { getDataFromServer, addToFavourites, removeFromFavourites } from "../service/api";
...
const MovieList = (props: Props) => {
    ...
    const [filteredItems, setFilteredItems] = useState<DataSetList[]>([]);
    const [toast, setToast] = useState<ToastObject[]>([]);
    const [search, setSearch] = useState<string>("");
    const [receivedData, setReceivedData] = useState<DataSetList[]>([]);
    ...
    const filterItems = (searchString: string, data: DataSetList[]) => {
        if (searchString !== "") {
    ...
    const handleFavourites = async (movie: DataSetList) => {
        try {
            if (section === "favourite") {
                ...
                const movieAlreadyPresent = favouriteMovies.filter((data: { title: string; }) => data.title === movie.title);
